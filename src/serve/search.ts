...
export async function serveSearchPage(request: Request, env: Env, username?: string): Promise<Response> {
  ...
  const search = { query, kind, cursor, username };
  if (!query && !username) return html(renderSearchPage(search), 200);

  try {
    const result = await withCookieSession(env.NITTER_SESSIONS, async (session) => {
      const rawQuery = `${username ? `from:${username} ` : ""}${query} include:nativeretweets`.trim();
      if (!username) {
    ...
    return html(renderSearchPage(search, result.timeline, result.profile, result.photos), 200);
  } catch (error) {
